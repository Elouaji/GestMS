...
public class MachineController extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
    	...
    	if (request.getParameter("op") != null) {
            ...
            else if (request.getParameter("op").equals("RechercheDate")) {
                response.setContentType("application/json");
            ...
            else if(request.getParameter("op").equals("findMachines"))
            {
            	response.setContentType("application/json");
            	List<Machine> machines = ms.findMachines(Integer.parseInt(request.getParameter("idSalle")));
                Gson json = new Gson();
                response.getWriter().write(json.toJson(machines));
                System.out.println("findMachines");
            }
            ...
            else if (request.getParameter("op").equals("statistics"))
            {
            	response.setContentType("application/json");
                Gson json = new Gson();
                RepGson repGson = new RepGson(us.usersCount(),ms.machineCount(),ss.salleCount());
